// https://opentdb.com/api.php?amount=10&number=21&difficulty=easy
...
class Quiz {
  ...
  getApi() {
    return ` https://opentdb.com/api.php?amount=${this.number}&number=${this.category}&difficulty=${this.difficulty}`
  }
  ...
  showResult() {
    return `
      ...
      >
        <h2 class="mb-0">
             ${this.score == this.number ? `congratulation`
...
class Question {
  ...
  checkAnswers(choice) {
    ...
    if (!this.isAnswered) {
      ...
      if (choice.textContent === this.correct_answer) {
